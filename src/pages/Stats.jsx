...
export default function Stats() {
        ...
        const fetchStats = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_URL}/stats?monthOffset=${monthOffset}`,
                );
                setData(res.data);
            } catch (error) {
    ...
    const salesTrend = data.historicalAverages
        ? calculateTrend(
              overview.totalSales,
              data.historicalAverages.avgSalesPerMonth,
          )
        : null;
    const profitTrend = data.historicalAverages
        ? calculateTrend(
              overview.totalProfit,
              data.historicalAverages.avgProfitPerMonth,
          )
        : null;
    const profitMargin =
    ...
    const unpaidTotalValue =
        (overview.unpaidValue || 0) + (overview.unsettledValue || 0);
    const hasAlerts = unpaidCount > 0;

    // Derived Analytics
    const totalOrdersCount = overview.fulfilledOrders + unpaidCount;
    const aov =
        totalOrdersCount > 0 ? overview.totalSales / totalOrdersCount : 0;

    const revenueSubtitleItems = [];
    if (overview.totalCost > 0)
        revenueSubtitleItems.push(
            `Cost: ${formatCurrency(overview.totalCost)}`,
        );
    if (overview.totalDiscount > 0)
        revenueSubtitleItems.push(
            `Disc: ${formatCurrency(overview.totalDiscount)}`,
        );
    const revenueSubtitle =
        revenueSubtitleItems.length > 0
            ? revenueSubtitleItems.join(" • ")
            : undefined;
                    ...
                    <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
                        <div>
                            ...
                            <p className="text-slate-500 font-medium mt-2 sm:text-lg">
                                Here is how your business is performing at a
                                glance.
                            </p>
                        </div>
                        ...
                        <div className="bg-white p-6 sm:p-8 rounded-4xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100/60 flex flex-col min-h-[380px] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
                            ...
                            <div className="flex-1 w-full space-y-5">
                                {topFlowers && topFlowers.length > 0 ? (
                                    ...
                                    <EmptyState
                                        message="No flower sales data yet."
                                        icon={FiShoppingBag}
                                    />
                                )}
                            </div>
                                        ...
                                        >
                                            ...
                                            <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden flex relative">
                                                ...
                                                <div
                                                    ...
                                                    style={{
                                                        width: `${(loc.totalSales / maxLocationSales) * 100}%`,
                                                    }}
                                                >
                                ...
                                {recentActivity && recentActivity.length > 0 ? (
                                    recentActivity.map((activity, idx) => (
                                        ...
                                        >
                                            {/* Timeline Node Icon */}
                                            <div
                                                className={`absolute -left-[35px] top-0 w-4 h-4 rounded-full border-4 border-white shadow-sm transition-transform group-hover:scale-125 ${
                                                    activity.paid &&
                                                    activity.settled
                                                        ? "bg-emerald-500"
                                                        : activity.paid ||
                                            ...
                                            <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl group-hover:shadow-md group-hover:-translate-y-1 transition-all">
                                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        {activity.paid ? (
                                                            <span className="px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-extrabold bg-emerald-100 text-emerald-700 flex items-center gap-1">
                                                                <FiCheckCircle
                                                                    size={10}
                                                                />{" "}
                                                                Paid
                                                            ...
                                                            <span className="px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-extrabold bg-rose-100 text-rose-700">
                                                                Unpaid
                                                            </span>
                                                        )}
                                                        {activity.settled ? (
                                                            <span className="px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-extrabold bg-blue-100 text-blue-700 flex items-center gap-1">
                                                                <FiCheckCircle
                                                                    size={10}
                                                                />{" "}
                                                                Settled
                                                            ...
                                                            <span className="px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-extrabold bg-amber-100 text-amber-700">
                                                                Pending
                                                                Settlement
                                                            </span>
                                                        )}
                            ...
                            <div className="space-y-4 flex-1">
                                ...
                                data.topDebtors.length > 0 ? (
                                    data.topDebtors.map((debtor, idx) => (
                                        <div
                                            key={idx}
                                            className="group flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 hover:border-rose-200 hover:shadow-md transition-all cursor-default relative overflow-hidden"
                                        >
...
function AnimatedMetricCard({
    ...
    return (
        ...
        >
            ...
            <div
                className={`absolute -right-10 -bottom-10 w-32 h-32 rounded-full bg-linear-to-br ${gradient} opacity-50 blur-2xl group-hover:opacity-100 transition-opacity duration-500`}
            ></div>
            ...
            <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors duration-300 border ${colorStyles[color]} relative z-10`}
            >
                <Icon size={24} />
            </div>
