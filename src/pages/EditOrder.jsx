                                ...
                                <div key={index} className="flex flex-col sm:flex-row gap-4 items-start sm:items-center p-4 bg-slate-50 rounded-xl border border-slate-200 shadow-sm relative">
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                                        <div>
                                            ...
                                            <input type="number" min="0" placeholder="0" className="w-24 p-2 bg-slate-50 border border-slate-200 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none font-semibold text-slate-800" value={item.cost === 0 || item.cost === '' ? '' : item.cost} onFocus={(e) => e.target.select()} onChange={(e) => handleItemChange(index, 'cost', e.target.value)} />
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Price (Rs.)</label>
                                            <input type="number" min="0" placeholder="0" className="w-24 p-2 bg-slate-50 border border-slate-200 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none font-semibold text-slate-800" value={item.price === 0 || item.price === '' ? '' : item.price} onFocus={(e) => e.target.select()} onChange={(e) => handleItemChange(index, 'price', e.target.value)} />
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Qty</label>
                                            <input type="number" min="1" placeholder="0" className="w-16 p-2 bg-slate-50 border border-slate-200 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none font-semibold text-slate-800 text-center" value={item.qty === 0 || item.qty === '' ? '' : item.qty} onFocus={(e) => e.target.select()} onChange={(e) => handleItemChange(index, 'qty', e.target.value)} onBlur={() => { if (!item.qty || item.qty <= 0) handleItemChange(index, 'qty', 1); }} />
                                        </div>
                        ...
                        <div className="border-t border-slate-100 pt-6">
                            ...
                            {isCustomMode ? (
                                <form onSubmit={addCustomToCart} className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-wrap gap-3 items-end">
                                    ...
                                    <div className="w-24"><label className="text-xs font-semibold text-slate-500 mb-1 block">Cost</label><input type="number" placeholder="0" className="w-full p-2.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={customItem.cost === 0 || customItem.cost === '' ? '' : customItem.cost} onFocus={(e) => e.target.select()} onChange={e=>setCustomItem({...customItem, cost: e.target.value})} /></div>
                                    <div className="w-24"><label className="text-xs font-semibold text-slate-500 mb-1 block">Price</label><input type="number" required placeholder="0" className="w-full p-2.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={customItem.price === 0 || customItem.price === '' ? '' : customItem.price} onFocus={(e) => e.target.select()} onChange={e=>setCustomItem({...customItem, price: e.target.value})} /></div>
                                    <div className="w-20"><label className="text-xs font-semibold text-slate-500 mb-1 block">Qty</label><input type="number" required placeholder="0" className="w-full p-2.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={customItem.qty === 0 || customItem.qty === '' ? '' : customItem.qty} onFocus={(e) => e.target.select()} onChange={e=>setCustomItem({...customItem, qty: e.target.value})} onBlur={() => { if (!customItem.qty || customItem.qty <= 0) setCustomItem({...customItem, qty: 1}); }} /></div>
