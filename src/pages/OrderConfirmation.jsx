    ...
    if (confirmedOrder) {
        return (
            <div className="w-full max-w-2xl mx-auto space-y-6 flex flex-col items-center">
                ...
                <div className="w-full flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 p-4 bg-white rounded-xl shadow-sm border border-slate-200 justify-center">
                    <button
                        onClick={handleDownloadBill}
                        className="w-full sm:w-auto flex justify-center items-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-xl border border-indigo-700 shadow-sm hover:bg-indigo-700 font-medium transition-all hover:-translate-y-0.5"
                    >
                        <FiDownload /> Download Bill Image
                    </button>
                    <button
                        onClick={() => navigate("/new-order")}
                        className="w-full sm:w-auto flex justify-center items-center gap-2 bg-white text-slate-700 px-6 py-2.5 rounded-xl border border-slate-200 shadow-sm hover:bg-slate-50 font-medium transition-all hover:-translate-y-0.5"
                    >
                        Create Another Order
                    </button>
                    <button
                        onClick={() => navigate("/")}
                        className="w-full sm:w-auto flex justify-center items-center gap-2 bg-slate-100 text-slate-600 px-6 py-2.5 rounded-xl border border-slate-200 shadow-sm hover:bg-slate-200 font-medium transition-all hover:-translate-y-0.5"
                    >
                        Home
                    </button>
                </div>
                    ...
                    >
                        ...
                        <div className="text-center mt-10 pt-4 border-t border-dashed border-slate-300">
                            ...
                            <p className="text-xs text-slate-400 mt-1">
                                Visit us again or contact support for help.
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div className="w-full max-w-4xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
                            <span className="w-6 h-6 rounded bg-indigo-100 text-indigo-700 flex items-center justify-center text-sm">
                                1
                            </span>
                            Buyer Details
                        </h2>
                        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 shadow-inner">
                            <p className="font-bold text-slate-800 text-lg flex items-center gap-2">
                                {buyer.name}
                                {buyer.businessName && (
                                    <span className="bg-slate-200 text-slate-700 text-xs px-2 py-0.5 rounded-full font-medium tracking-wide uppercase">
                                        {buyer.businessName}
                                    </span>
                                )}
                            </p>
                            <p className="text-slate-600 mt-1 font-medium bg-white inline-block px-3 py-1 rounded-md border border-slate-100 shadow-sm text-sm">
                                {buyer.telephone} • {buyer.location}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <div className="space-y-4">
                            {cart.map((item, index) => (
                                <div
                                    key={index}
                                    className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center p-4 bg-slate-50 rounded-xl border border-slate-200 shadow-sm hover:border-indigo-200 transition-colors"
                                >
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 self-start sticky top-6">
                    ...
                    <div
                        className="flex justify-between items-center p-4 rounded-xl border border-slate-200 bg-slate-50 mb-6 cursor-pointer hover:bg-slate-100 transition-colors shadow-sm"
                    ...
                    >
                        ...
                        <div
                            className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors ${isSettled ? "bg-emerald-500" : "bg-slate-300"}`}
                        >
                            <span
                                className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform shadow-sm ${isSettled ? "translate-x-6" : "translate-x-1"}`}
                            />
                        </div>
                    </div>
                    ...
                    >
                        {isSubmitting ? (
                            ...
                            <>
                                <FiCheck size={24} /> Confirm Order
                            </>
                        )}
                    </button>
                    <p className="text-center text-xs text-slate-400 mt-4 font-medium">
                        Please double check pricing before confirming.
                    </p>
                </div>
            </div>
        </div>
